...
For current AI Lab spiking neurons projects using multiple ARM32 microcontrollers, FSM code is replacing more conventional loops and IO handler functions for autonomous neuromorphic simulations. These MCUs are programmed in CircuitPython using builtin state, transition, and IO functions, classes and objects.

CircuitPython developer **adafruit** has provided a helpful comparative [guide to programming FSMs](https://cdn-learn.adafruit.com/downloads/pdf/circuitpython-101-state-machines.pdf) for a NY-style New Year's Eve dropping ball countdown display controller. Other common examples for FSM code development focus on autonomous machine controllers for [traffic lights](https://python-statemachine.readthedocs.io/en/latest/readme.html) and transit turnstiles.
