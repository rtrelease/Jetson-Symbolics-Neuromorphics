...
***The images below show first tests of a new Jetson-compiled [CLIPS 6.40 (2/21)](https://sourceforge.net/projects/clipsrules/)***  *Lisp/Scheme like symbolic programming environment* running **qualitative  process (QP) model** simulations on a Jetson Xavier NX.  CLIPSJNI is executing though the OpenJDK Java virtual machine via a .*jar* app launched from a REPL command line. The required CLIPS Java/C .*so* function library was compiled for Jetson's specific aarch64 OpenJDK 11 installation.
